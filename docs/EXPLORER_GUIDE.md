...
이 문서는 코드의 책임과 실제 데이터 연결을 따라가는 안내서다. 기준은 README와 같은 `36c3d716` 스냅샷이다. 전체 파일 목록을 확인하고 자체 코드 142개, 게임 씬, 프리팹·ScriptableObject·입력·저장 데이터의 연결을 중심으로 분석했다. 외부 SDK와 대량 이미지 리소스는 위치와 의존 관계를 구분했다.
